...
class SimpleNet(torch.nn.Module):
    ...
    def test(self, training_data, test_data):
        ...
        anomaly_labels = test_data.dataset.labels
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    i_iter += 1
                    img = data_item["image"]
                    aug_img = data_item["aug_image"]
                    masks = data_item["mask"]
                    img = img.to(torch.float).to(self.device)
                    aug_img = aug_img.to(torch.float).to(self.device)
                    masks = masks.to(self.device)

                    true_feats, patch_shapes = self._embed(img, evaluation=False)
                    sync_feats, patch_shapes = self._embed(aug_img, evaluation=False)
                    if self.pre_proj > 0:
                        true_feats = self.pre_projection(true_feats)
                        sync_feats = self.pre_projection(sync_feats)
                    ...
                    scores = self.discriminator(torch.cat([sync_feats, true_feats, fake_feats]))

                    score_gap = len(sync_feats)
                    sync_scores = scores[:score_gap]
                    true_scores = scores[score_gap:2*score_gap]
                    fake_scores = scores[2*score_gap:]

                    th = self.dsc_margin
                    p_true = (true_scores.detach() >= th).sum() / len(true_scores)
                    p_sync = (sync_scores.detach() < -th).sum() / len(sync_scores)
                    ...
                    self.logger.logger.add_scalar(f"p_sync", p_sync, self.logger.g_iter)

                    # true_loss = torch.clip(-true_scores + th, min=0)
                    # fake_loss = torch.clip(fake_scores + th, min=0)
                    # sync_loss = torch.clip(sync_scores + th, min=0)
                    # loss = true_loss.mean() + fake_loss.mean()

                    scales = patch_shapes[0]
                    pred = torch.cat([true_scores, fake_scores, sync_scores], dim=1)
                    pred = pred.reshape(-1,scales[0], scales[1],3)
                    pred = torch.permute(pred, (0,3,1,2))

                    gt_label = torch.zeros_like(pred, dtype=torch.float32).to(self.device)
                    resize_mask = F.interpolate(masks.unsqueeze(1).float(), pred.shape[2:], mode='nearest').squeeze() 
                    gt_label[:,0,:,:]=1
                    gt_label[:,2,:,:]=1-resize_mask
                    loss = self.loss_fn(pred, gt_label)
                    
                    if i_iter%500<10:
                        xi_iter = i_iter%500
                        mean = np.array([[0.485, 0.456, 0.406] for ix in range(len(img))])
                        std = np.array([[0.229, 0.224, 0.225] for ix in range(len(img))])
                        ximg = img.cpu().numpy() * std[:,:,None,None] + mean[:,:,None,None]
                        xaug = aug_img.cpu().numpy() * std[:,:,None,None] + mean[:,:,None,None]
                        xmask = masks.cpu().numpy()
                        xpred = pred.cpu().detach().numpy()
                        xgt   = gt_label.cpu().numpy()
                        fimg = np.transpose(ximg,(0,2,3,1))
                        faug = np.transpose(xaug,(0,2,3,1))
                        fpred = np.transpose(xpred,(0,2,3,1))
                        fgt = np.transpose(xgt,(0,2,3,1))
                        cv2.imwrite(f'./results/fimg_{xi_iter}.png', (fimg[0]*255+0.1).astype(np.uint8))
                        cv2.imwrite(f'./results/faug_{xi_iter}.png', (faug[0]*255+0.1).astype(np.uint8))
                        cv2.imwrite(f'./results/mask_{xi_iter}.png', (xmask[0]*255).astype(np.uint8))
                        cv2.imwrite(f'./results/gt_{xi_iter}.png', (fgt[0]*255).astype(np.uint8))
                    ...
                    all_p_sync.append(p_sync.cpu().item())
                ...
                all_p_sync = sum(all_p_sync) / len(input_data)
                ...
                pbar_str += f" p_true:{round(all_p_true, 3)} p_fake:{round(all_p_fake, 3)} p_sync:{round(all_p_sync, 3)}"
